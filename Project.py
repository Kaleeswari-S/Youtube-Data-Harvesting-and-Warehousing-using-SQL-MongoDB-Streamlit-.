...
client = MongoClient("mongodb+srv://kaleeswariramkumar25:ram@kaleeswari24.stlokdg.mongodb.net/?retryWrites=true&w=majority")
db = client['Youtube']
coll = db['Channels']

#                                               ---------Connection with Postgresql DATABASE---------

Integration=pg.connect(host="localhost",
                    user="postgres",
                    password="ram7",
                    database= "Youtube1",
                    port = "5432")
...
api_key = "AIzaSyAX2gy1qjfm3sz1YEwYBnPgp_sAyZ0zyKc"
...
def insert_into_channels():
    ...
    for i in coll.find({"Channel Details.channel_name": user_input}, {'_id': 0}):
        ...
        try:
            ...
            Integration.commit()
            st.success("✅Data Transformed to SQL, Successfully!")

        except pg.IntegrityError as e:
            # Handle unique key violation error here
            st.error(" 🚫THIS Data DETAILS ARE ALREADY EXIST!, KINDLY CHECK IT.")
...
if option == "Extract and Transform":
    tab1,tab2 = st.tabs(["EXTRACT AND UPLOAD", "TRANSFORM"])
    ...
    with tab1:
        ...
        if ch_id and st.button("Extract and Upload Data"):
...
if option == 'View and Analyse':
    questions = st.selectbox('CLICK THE QUESTION THAT YOU WOULD LIKE TO QUERY',
        ...
        '10. What is the total number of likes for each video, and what are  their corresponding video names?'])
    ...
    if questions == '10. What is the total number of likes for each video, and what are  their corresponding video names?':
        query10 = 'select video_title, video_likes as Maximum_likes from channels_yt c join videos_yt v  on c.channel_id = v.channel_id order by video_likes desc limit 1'
        connect.execute(query10)
        result10 = connect.fetchall()
        column_names = [column[0] for column in connect.description]
        table10 = pd.DataFrame(result10, columns=column_names)
        st.table(table10)
